Shader "Custom/ScreenPos"
{
    Properties
   {
      ...
      [Header(Grid)]
      _ScaleWidth("ScaleWidth", Float) = 1.0
      _ScaleHeight("ScaleHeight", Float) = 1.0
   ...
   SubShader
   {
      ...
      Pass
      {
         ...
         fixed4 frag (v2f i) : SV_Target
         {
            ...
            float logMappedScaleWidth = _ScaleWidth / pow(10, ceil(log10(_ScaleWidth)));
            float logMappedScaleHeight = _ScaleHeight / pow(10, ceil(log10(_ScaleHeight)));
            ...
            float localScaleWidth = 1 / logMappedScaleWidth;
            float localScaleHeight = 1 / logMappedScaleHeight;
            ...
            float fadeWidth =  pow(1 - remap(logMappedScaleWidth, 0.1, 1, 0.00001, 0.99999), _SecondaryFadeInSpeed);
            float fadeHeight =  pow(1 - remap(logMappedScaleHeight, 0.1, 1, 0.00001, 0.99999), _SecondaryFadeInSpeed);

            float2 pos;

            pos.x = floor(frac((i.uv.x - 0.5 * _Thickness) * localScaleWidth) + _Thickness * localScaleWidth);
            pos.y = floor(frac((i.uv.y - 0.5 * _Thickness) * localScaleHeight) + _Thickness * localScaleHeight);

            if (pos.x == 1) {
               col.a = max((1 - fadeWidth), fadeWidth);
            } else if	(pos.y == 1){
			col.a = max((1 - fadeHeight), fadeHeight);
			}
			else {
               pos.x = floor(frac((i.uv.x - 0.5 * _Thickness) * 10.0 * localScaleWidth) + _Thickness * 10.0 * localScaleWidth);
               pos.y = floor(frac((i.uv.y - 0.5 * _Thickness) * 10.0 * localScaleHeight) + _Thickness * 10.0 * localScaleHeight);
